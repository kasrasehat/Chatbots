...
def candidate_retriever():
    """
    Directs the chatbot to navigate the candidate selection process.

    This tool is responsible for guiding the system towards the node that creates 
    search criteria based on the user's question. It does not return data itself but 
    ensures the correct workflow is followed to retrieve relevant candidates.

    Purpose:
        - Determines the need for a candidate search process.
        - Passes control to the criteria-building node.
        - Ensures relevant data is gathered before searching for candidates.

    Notes:
        - This tool does not return any direct results but plays a key role in the 
          overall candidate retrieval pipeline.
    """
    return "This tool directs the system to initiate the candidate search process by defining search criteria."
...
class Agent:
    def __init__(self, vectordb , model, tools, aux_tools, checkpointer, system="", user_input='', language='English'):
        self.system = system
        ...
        graph.add_node("action1", self.take_action1)
        graph.add_node("search_data", self.call_rag)
        graph.add_node("candidate_finder", self.find_candidate)
        graph.add_node("llm1", self.call_openai1)
        
        
        # Conditional edges and flow control
        graph.set_entry_point("llm")
        graph.add_conditional_edges("llm", self.exists_action, {'find_candidate':"candidate_finder",'take_action': "action", 'retrieve_data': "search_data", False: END})
        # graph.add_conditional_edges("candidate_finder", self.exists_action1, {'take_action': "action1", False: END})
        graph.add_edge("action", "llm")
        graph.add_edge("search_data", "llm")
        graph.add_edge("candidate_finder", "llm")
        # graph.add_edge("action1", "llm1")
        ...
        self.tools_aux = {t.name: t for t in aux_tools}
        self.model_aux = model.bind_tools(aux_tools)
    ...
    def exists_action(self, state: AgentState):
        ...
        if len(result.tool_calls) > 0 or state.get("flow_state", None) == "candidate_retriever" and not isinstance(state['messages'][-2], ToolMessage):
            
            if result.tool_calls[0]['name'] == 'data_retriever': 
                return 'retrieve_data'
            
            elif result.tool_calls[0]['name'] == 'candidate_retriever' or state.get("flow_state", None) == "candidate_retriever":
    ...
    def exists_action1(self, message):
    ...
    def take_action1(self, state: AgentState):
        tool_calls = state['messages'][-1].tool_calls
        results = []
        for t in tool_calls:
            tool_name = t['name']
            tool_args = t['args']
            print(f"Calling: {tool_name} with args: {tool_args}")
            result = self.tools_aux[tool_name](*tool_args if isinstance(tool_args, list) else [tool_args])
            results.append(ToolMessage(tool_call_id=t['id'], name=tool_name, content=str(result)))
        print("Back to the model!")
        return {'messages': results}
    ...
    def call_openai1(self, state: AgentState):
        ...
        if self.system:
            system_message = '''
                            ...
                            Your goal is to strictly follow this format when presenting candidates:
                            {
                                "text": {
    ...
    def call_rag(self, state: AgentState):
        ...
        if self.vectordb:
            ...
            if len(filtered_docs)==0:
                ...
                tool_name = t['name']
                tool_args = t['args']
                context = tavily_client.get_search_context(query=tool_args)
                results = []
                print(f"Calling: tavily serach with args: {tool_args}")
                result = context
                results.append(ToolMessage(tool_call_id=t['id'], name=tool_name, content=str(result)))
                print("Back to the model!")
                return {'messages': results}
    ...
    def find_candidate(self, state: AgentState):
        system_msg = (
                        '''You are a hiring assistant responsible for gathering candidate search criteria from an employer.
        ...
        message = self.model_aux.invoke(messages)
        if not self.exists_action1(message):
            tool_calls = state['messages'][-1].tool_calls
            t = tool_calls[0]
            tool_name = t['name']
            tool_args = t['args']
...
def get_response(user_input, vectordb, language, state):
    ...
    prompt = """
                You are a smart assistant of berry on mars company tasked with guiding the user through changing their password or serach any data or find appropriate candidate based on provided data, if they want. 
                    ...
                    you have the following tools at your disposal for finding candidate:
                        candidate_retriever: Retrieve the appropriate candidate.    

                    When appropriate, call these tools to proceed through the steps. under no circumstances Do you answer questions from your own knowledge.
    ...
    tools = [ask_email, check_email_format, send_code, verify_email, ask_password, retype_password, conform_passwords, record_password, data_retriever, candidate_retriever]
    aux_tools = [search_candidate]

    # messages = conversation_history + [HumanMessage(content=user_input)]
    with SqliteSaver.from_conn_string(":memory:") as memory:
        abot = Agent(vectordb, model, tools, aux_tools, checkpointer=memory, system=prompt, user_input=user_input, language=language)
...
def get_demo():
    with gr.Blocks() as demo:
        ...
        def on_send_button(user_input, vectordb_state, agent_state, language):
            ...
            agent_state = get_response(user_input, vectordb_state, language, state)
