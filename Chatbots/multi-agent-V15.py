...
class Agent:
    def __init__(self, vectordb , model, tools, checkpointer, system="", user_input='', language='English'):
        ...
        graph.add_conditional_edges("candidate_finder", self.exists_action, {'take_action': "action", False: END})
    ...
    def find_candidate(self, state: AgentState):
        system_msg = (
                        ...
                        4. Once the JSON is fully populated, **call the `search_candidate` tool** to fetch candidates based on the gathered criteria.
        ...
        conversation = [{"role": "system", "content": system_msg}]
        messages = conversation + [{"role": "user", "content": f"{self.user_input}"}]
        message = self.model.invoke(messages)
        return {'messages': [message]}
...
def get_response(user_input, vectordb, language, state):
    ...
    tools = [ask_email, check_email_format, send_code, verify_email, ask_password, retype_password, conform_passwords, record_password, data_retriever, candidate_retriever, search_candidate]
