...
def get_response(audio, vectordb, language):
    ...
    if language == 'German':
        ...
        system_msg = (
            "You are an english AI assistant of berry on mars company that helps answer user questions based on the provided context and your own knowledge. "
            "Do not forget to introduce yourself at the beginning of your conversation."
    ...
    chatbot_audio = text_to_speech(response.choices[0].message.content)
    return user_input, response.choices[0].message.content, chatbot_audio
    ...
    with gr.Blocks() as demo:
        ...
        with gr.Row():
            ...
            with gr.Column():
                ...
                user_input = gr.Audio(sources="microphone", type="filepath", label="Speak your query", format="mp3")
        ...
        def on_process_button(url, depth, max_url, language, uploaded_files):
            ...
            return vectordb, processing_status_text, gr.update(interactive=process_button_interactive)
        ...
        def on_send_button(user_input, vectordb_state, language):
            ...
            return response[0], response[1], response[2], gr.update(value=None)

        # Link Buttons to Handlers
        process_button.click(
            fn=on_process_button,
            inputs=[url_input, depth_slider, max_url_slider, language_radio, uploaded_files],
            outputs=[vectordb_state, processing_status, process_button]
        )

        send_button.click(
            fn=on_send_button,
            inputs=[user_input, vectordb_state, language_radio],
            outputs=[user_text_output,  chatbot_response_output, chatbot_audio_output, user_input]
        )
